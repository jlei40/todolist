<template>
    <div class="task-details">
      <a :href="tasks_list_url" class="task-link">Task List</a><br/>
      <a :href="tasks_update_url" class="task-link">Update Tasks</a><br/>
      <a :href="tasks_delete_url" class="task-link">Delete Tasks</a><br/>
      <h1 class="task-name">{{ tasks.name }}</h1>
      <div class="tag-list">
        <span v-for="tag in tasks.tags" class="tag">{{ tag.name }}</span>
      </div>
      <p class="task-description">Description: {{ tasks.description }}</p>
      <p class="task-date">Date: {{ convert_date_to_string(tasks.date) }}</p>
    </div>
  </template>
